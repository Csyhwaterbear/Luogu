...
/* ===== 解决一个测试用例 ===== */
bool func(const vector<int>& list, long long limit, int M) {
	long long cur = 0;
	int cnt = 1; // 至少有一段
	for (int x : list) {
		if (x > limit) return false; // 单个元素都放不下
		if (cur + x <= limit) {
			cur += x;
		} else {
			++cnt;
	...
	long long l = *max_element(all(list)); // 下界：至少容纳最大元素
	long long r = 0; // 上界：全部放一段
	for (int x : list) r += x;

	while (l < r) {
		long long mid = l + (r - l) / 2;
		if (func(list, mid, M)) r = mid;
		else l = mid + 1;
	}
	cout << l << '\n';
}
