...
int n;
vector<int> Col;
vector<bool> usedCol, diag_r, diag_l;
int cnt = 0;
...
void dfs(int r) {
	...
	for (int c = 1; c <= n; c++) {
		if (!usedCol[c] && !diag_r[r + c] && !diag_l[r - c + n]) {
			Col[r] = c;
			usedCol[c] = diag_r[r + c] = diag_l[r - c + n] = true;
			dfs(r + 1);
			usedCol[c] = diag_r[r + c] = diag_l[r - c + n] = false;
		}
	}
}
...
void solve() {
	...
	diag_r.assign(2 * n + 1, false);
	diag_l.assign(2 * n + 1, false);
	dfs(1);
	cout << cnt << '\n';
}
