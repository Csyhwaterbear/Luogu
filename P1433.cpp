...
void solve() {
	int n;
	cin >> n;
	vector<pair<double, double>> list(n+1);
	for (int i = 1; i <= n; i++)
	...
	for (int i = 0; i <= n; i++) {
		for (int j = 0; j <= n; j++) {
			double dx = list[i].first  - list[j].first;
			double dy = list[i].second - list[j].second;
			dist[i][j] = sqrt(dx * dx + dy * dy);
		}
	}
	const double INF = 1e100;
	int SIZE = 1 << n;
	vector<vector<double>> dp(SIZE, vector<double>(n+1, INF));
	for (int i = 1; i <= n; i++)
		dp[1 << (i-1)][i] = dist[0][i];
