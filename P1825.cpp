...
void solve() {
	...
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < M; j++) {
			if (grid[i][j] == '@')
				...
				portal[grid[i][j] - 'A'].push_back({i, j});
		}
	}
	...
	while (!q.empty()) {
		...
		for (int dir = 0; dir < 4; dir++) {
			...
			if ('A' <= cell && cell <= 'Z') {
				int id = cell - 'A';
				pii a = portal[id][0], b = portal[id][1];
				pii to = (a.first == nx && a.second == ny) ? b : a;
				if (dist[to.first][to.second] > cur + 1) {
					dist[to.first][to.second] = cur + 1;
					q.push(to);
				}
			} else {
				if (dist[nx][ny] > cur + 1) {
