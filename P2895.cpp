...
void solve() {
	...
	for(int i = 0; i < M; i++) {
		...
		if (x + 1 < MAXN) destroy[x + 1][y] = min(destroy[x + 1][y], t);
		if (x - 1 >= 0 ) destroy[x - 1][y] = min(destroy[x - 1][y], t);
		if (y + 1 < MAXN) destroy[x][y + 1] = min(destroy[x][y + 1], t);
		if (y - 1 >= 0 ) destroy[x][y - 1] = min(destroy[x][y - 1], t);
	}

	if (destroy[0][0] == 0){cout << -1 << endl; return;}
	...
	while (!q.empty()) {
		...
		if (destroy[x][y] == INF) {cout << t << endl; return;}

		for (int d = 0; d < 4; d++) {
			int nx = x + dx[d], ny = y + dy[d];
			if (nx < 0 || ny < 0) continue;
			if (nx >= MAXN || ny >= MAXN) {cout << t + 1 << endl; return;}
			if (!vis[nx][ny] && destroy[nx][ny] > t + 1) {
				vis[nx][ny] = true;
				q.push({nx, ny, t + 1});
			}
		}
